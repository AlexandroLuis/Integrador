...
import br.mypetsitter.model.ClientePrincipalFXML;
...
public class LoginController implements Initializable {
   ...
   // @FXML
    ...
    public void autenticar() throws SQLException {
    	...
    	/*if(ehAutonomo.getAccessibleText() == "C") {
    		Cliente cliente = new Cliente(idUsuario, senha);
    		ClienteDAOJDBC clienteDao = new ClienteDAOJDBC();
    		boolean existe = clienteDao.autenticarCliente(cliente);
    		if(existe) {
    			ClientePrincipalFXML clientePrincipal = new ClientePrincipalFXML();
    			try {
					clientePrincipal.start(new Stage());
					fechaJanela();
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	/*	ehAutonomo.setAccessibleText("C");
		ehAutonomo.selectedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(ehAutonomo.isSelected()) {
					ehAutonomo.setAccessibleText("A");
				} else {
					ehAutonomo.setAccessibleText("C");
				}
			}
